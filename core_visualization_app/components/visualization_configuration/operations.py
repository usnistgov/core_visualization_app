        ...
        for y in plot.y_parameters:
            ...
            for i in range(1, len(data_table)):
                if data_table[i][x_index] and data_table[i][y_index]:
                    xy_all_dicts[x+y].append({x: data_table[i][x_index], y: data_table[i][y_index]})
...
def multi_barchart(xy_all_dicts, plot):
    ...
    all_colors = ['#1f77b4', '#aec7e8', '#ff7f0e', '#ffbb78', '#2ca02c', '#98df8a', '#d62728', '#ff9896', '#9467bd',
    ...
    for i in range(0, len(y_parameters)):
        ...
        for couple in xy_dict:
            if couple[active_x] in classification:
                if couple[y_parameters[i]]:
                    ...
                    classification[couple[active_x]].append(float(couple[y_parameters[i]]))
            else:
                if couple[y_parameters[i]]:
        ...
        p.vbar(x=dodge('x_range', step, range=p.x_range), top='y_data', color=color,
               legend=y_name, width=width, source=source)
...
def pie_chart(xy_all_dicts, plot, x_value=None):
        ...
        if (active_x + plot.y_parameters[i]) in xy_all_dicts:
            ...
            for elt in xy_dict_list:
                if elt[active_x] not in active_x_values:
                    active_x_values.append(elt[active_x]) # active_x_values = list of all different project id if project id is active x

    plot = visualization_config_api.update_custom_parameters(plot.plot_name, plot.test_name, active_x_values)
    ...
    p = figure(plot_height=600, title='Pie Chart: '+plot.test_name+': '+active_x+' - '+x_value,
               tools="hover", tooltips="@number: @value", x_range=(-0.5, 1.0))

    p.wedge(x=0, y=1, radius=0.4,
            start_angle=cumsum('angle', include_zero=True), end_angle=cumsum('angle'),
            line_color="white", fill_color='color', legend='number', source=data)
...
def set_y_axis_parameters(list_of_dicts, project_filter):
    ...
    if "queries" in list_of_dicts:
        ...
        for query in list_of_dicts:
            ...
            path = query['query']
            y_parameters_part = get_y_parameters(path, template_id, project_filter, query_filter)
    ...
    if "query" in list_of_dicts:
        ...
        y_parameters = get_y_parameters(list_of_dicts['query'], template_id, project_filter, query_filter)
...
def get_y_parameters(path, template_id, project_filter, query_filter):
    ...
    for project in project_filter:
        results = query_database_api.execute_query(template_id, [json.dumps(project)], json.dumps(query_filter))
        for result in results:
            list_of_dicts_param = dict_utils.get_list_inside_dict(path, result.dict_content)
